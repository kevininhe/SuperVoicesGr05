...
def show_concurso(page,url):
    ...
    number_pages = 1
    if not current_user.is_authenticated:
        participantes = Participante.objects(concurso=concurso.id,convertido=True).order_by('-fechaCreacion').paginate(page=page, per_page=20).items
        number_pages = Participante.objects(concurso=concurso.id,convertido=True).count()
    else:
...
def participante_form(concurso_id):
    ...
    if form.validate_on_submit():
        ...
        participante.update(path_audio_origin=str(participante.id)+'.'+format_file)
        participante.save(cascade=True)
