...
def logout():
    userk = 'user_{}'.format(str(current_user.id))
    if cache.get(userk):
        cache.delete(userk)
    logout_user()
    return redirect(url_for('participantes.index'))
...
def load_user(user_id):
     # Crear la llave
    userk = 'user_{}'.format(str(user_id))
    # Intenta cargar el usuario desde la cache
    user_obj = pickle.loads(cache.get(userk)) if cache.get(userk) else None
    if user_obj is None:
        ...
        cache.set(userk,user_pkl,ex=600)
    return user_obj